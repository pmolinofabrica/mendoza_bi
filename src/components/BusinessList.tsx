...
export default function BusinessList() {
  ...
  const getSourceUrl = (sources: unknown, type: string) => {
    if (!Array.isArray(sources)) return null
    const source = sources.find((s: Record<string, unknown>) => s.type === type)
    return source ? (source.url as string) : null
  }
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {businesses.map((bz) => {
            ...
            return (
              <div key={bz.id as string} className="bg-neutral-900 p-5 rounded-3xl border border-neutral-800/80 hover:border-neutral-700 transition-colors flex flex-col h-full">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-white mb-1 line-clamp-1" title={bz.name as string}>{bz.name as string}</h3>

                  <div className="flex flex-wrap gap-2 mt-3">
                    {Boolean(bz.tipo_principal) && (
                      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-blue-900/30 text-blue-400 border border-blue-800/50">
                        ...
                        {String(bz.tipo_principal)}
                      </span>
                    )}
                    {Boolean(bz.localidad) && (
